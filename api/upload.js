...
module.exports = async (req, res) => {
  ...
  if (req.method === "GET") {
    return res.status(200).json({ message: "Catbox proxy is alive" });
  }
  ...
  try {
    ...
    const resultPromise = new Promise((resolve, reject) => {
      ...
      busboy.on("finish", async () => {
        try {
          if (!gotHash) return reject(new Error("Missing HASH"));
